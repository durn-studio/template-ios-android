...
/** Stable IDs the app uses to refer to leaderboards. Add one entry
 *  per leaderboard you create in App Store Connect + Play Games
 *  Console, then wire its Android ID into
 *  ANDROID_LEADERBOARD_IDS_BY_WORLD below. */
 ...
 *  the ASC UI. */
export const ALL_IOS_LEADERBOARD_IDS: readonly string[] = LEADERBOARD_CATEGORIES.map(
  (id) => iosLeaderboardIdForWorld(id),
);
