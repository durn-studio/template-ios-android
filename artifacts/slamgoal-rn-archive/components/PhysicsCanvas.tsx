...
import { getSprite } from "@/components/sprites";
      ...
      <Canvas style={{ width, height }}>
        ...
        <Group>
          {bodies.map((b, i) => {
            ...
            // Sprite dispatch — if the body's id matches a registered
            // sprite, render that. Otherwise fall back to the plain
            // Skia primitive (circle/rect with the body color).
            const sprite = getSprite(b.data.id);
            if (sprite) {
              const radius =
                b.data.shape === "circle"
                  ? (b.data.radius ?? 0.5) * pixelsPerMeter
                  : undefined;
              const halfW =
                b.data.shape === "box"
                  ? (b.data.halfW ?? 0.5) * pixelsPerMeter
                  : undefined;
              const halfH =
                b.data.shape === "box"
                  ? (b.data.halfH ?? 0.5) * pixelsPerMeter
                  : undefined;
              return (
